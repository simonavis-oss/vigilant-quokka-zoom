...
serve(async (req) => {
  ...
  if (jobError || !job || !['assigned', 'printing'].includes(job.status)) {
    return new Response(JSON.stringify({ error: "Job not found or not in a cancellable state" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
